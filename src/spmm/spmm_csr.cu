...
template <typename T>
__global__ void spmm_csr_kernel(CSRMatrix<T> A, T* B, T* C, unsigned int N) {
...
template <typename T>
void spmm_csr(CSRMatrix<T> A, T* B, T* C, int N) {
    dim3 blockSize(BLOCK_SIZE, BLOCK_SIZE);
    dim3 gridSize(cdiv(A.R, blockSize.x), cdiv(N, blockSize.y));

    spmm_csr_kernel<T><<<gridSize, blockSize>>>(A, B, C, N);
